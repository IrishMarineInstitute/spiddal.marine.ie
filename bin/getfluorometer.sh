...
# Oranmore, Galway
# Ireland
chmod 664 /home/dmuser/sites/spiddal.marine.ie/html/data/spiddal-fluorometer.json.new
nc -d 172.16.255.5 951 | head -n 1 | python -c 'import sys,json,datetime; timestamp = datetime.datetime.utcnow().isoformat()[:-3]+"Z"; [Date,Time,Fluorescence,CHL,Turbidity,NTU,Thermistor] = sys.stdin.readline().split(); CHLCount=int(CHL); NTUCount=int(NTU); CHL = round(0.0181 * (float(CHL)-49.0),4); NTU = round(0.0483*(float(NTU) - 50.0),4); print json.dumps({"Date": Date, "Time": Time, "FluorescenceWavelength": int(Fluorescence), "CHL": CHL, "TurbidityWavelength": int(Turbidity), "NTU": NTU, "Thermistor": int(Thermistor), "CHLCount": CHLCount, "NTUCount": NTUCount, "Timestamp": timestamp})' > /home/dmuser/sites/spiddal.marine.ie/html/data/spiddal-fluorometer.json.new && chmod 664 /home/dmuser/sites/spiddal.marine.ie/html/data/spiddal-fluorometer.json.new && mv /home/dmuser/sites/spiddal.marine.ie/html/data/spiddal-fluorometer.json.new /home/dmuser/sites/spiddal.marine.ie/html/data/spiddal-fluorometer.json
